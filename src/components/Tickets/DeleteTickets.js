...
const DeleteTickets = ({ history, match }) => {
  const bTContext = useContext(TicketsContext);
  const id = match.params.id;

  //       toast.error('😈😈  Not Found Your Search    Please Try Again');

  const { deleteTicket, error, clearError, clearState } = bTContext;

  const onDelete = () => {
    if (error === 'Not found Ticket') {
      toast.error('😈😈  Not Found Ticket    Please Try Again');
  ...
  return (
    <section className='text-gray-400 bg-gray-900 body-font'>
      <div className='container px-5 py-24 ml-64 flex flex-wrap'>
        <ToastContainer />

        <div className='flex flex-wrap -m-4'>
          <div className='p-4 w-full '>
            <div className='flex border-2 rounded-lg   bg-gray-700 border-gray-800 p-8 sm:flex-row flex-col'>
              <div className='w-16 h-16 sm:mr-8 sm:mb-0 mb-4 inline-flex items-center justify-center rounded bg-white text-indigo-400 flex-shrink-0'>
                <svg
                  height='512pt'
              ...
              <div className='flex-grow'>
                ...
                <p>Hey Admin ! Are Want To Delete?</p>
